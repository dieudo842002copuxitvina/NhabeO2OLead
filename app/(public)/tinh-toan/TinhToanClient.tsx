...
import { generateBOM, type BOMResult } from "@/lib/bom";
...
/* ─────────────────────────────────────────────────────────────────────────────
 * URL PARAMS CAPTURE COMPONENT
 * ───────────────────────────────────────────────────────────────────────────── */

function UrlParamsCapture({ onCapture }: { onCapture: (params: UrlParams) => void }) {
...
function ResultsDisplay({ 
  ...
  return (
    <div className="space-y-6">
      {/* Dealer Assignment Banner */}
      {assignedDealer && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-blue-50 border border-blue-200">
          <Store className="w-6 h-6 text-blue-600 shrink-0" />
          <div>
            <p className="font-semibold text-blue-800">Đại lý được phân công</p>
            <p className="text-sm text-blue-600">Lead sẽ được gửi trực tiếp đến đại lý này (ID: {assignedDealer.slice(0, 8)}...)</p>
          </div>
        </div>
      )}
      ...
      {productInterest && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-purple-50 border border-purple-200">
          ...
          <div>
            <p className="font-semibold text-purple-800">Sản phẩm quan tâm</p>
            <p className="text-sm text-purple-600">{productInterest}</p>
          </div>
        </div>
      )}
        ...
        <CardContent className="p-4 space-y-4">
          ...
          <div className="space-y-3">
            <h4 className="text-sm font-semibold text-slate-700">
              {assignedDealer 
                ? "Nhận báo giá từ đại lý được phân công" 
                : "Nhận tư vấn từ đại lý gần nhất"
              }
            </h4>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400">
                {assignedDealer 
                  ? "Đã có đại lý được phân công sẵn" 
                  : "Giúp hệ thống phân bổ đại lý gần bạn nhất"
                }
              </p>
            </div>
          </div>
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                {assignedDealer ? "Gửi cho Đại lý được phân công" : "Gửi cho Đại lý"}
              </>
            )}
          </Button>
          ...
          {bom && bom.items.length > 0 && (
            ...
            >
              ...
              Liên hệ qua Zalo để nhận báo giá chi tiết
            </Button>
          )}
        </CardContent>
      </Card>
...
export default function TinhToanClient() {
    ...
    try {
      ...
      const actionResult = await submitCalculatorLeadWithRouting({
        ...
        calculatorData: {
          ...
          emitterCount: input.emitterCount,
          results: result,
          productInterest: urlParams.productInterest,
        },
        bomItems: bom ? bom.items.map(item => ({
          item: item.name,
          qty: item.quantity,
          unit: item.unit,
        ...
        totalCost: bom?.totalCost || result.estimatedCostVND,
        // Pre-assigned dealer from URL params (bypasses Round-Robin)
        assignedDealer: urlParams.assignedDealer || undefined,
        // UTM Tracking
        utmSource: urlParams.utmParams.utm_source,
        utmMedium: urlParams.utmParams.utm_medium,
        utmCampaign: urlParams.utmParams.utm_campaign,
        utmContent: urlParams.utmParams.utm_content,
        utmTerm: urlParams.utmParams.utm_term,
      });
      ...
      if (actionResult.success && actionResult.leadId) {
        ...
        toast({
          title: "Gửi thất bại",
          description: actionResult.error || "Đã xảy ra lỗi, vui lòng thử lại",
          variant: "destructive",
        });
      }
    } catch (error) {
          ...
          <div className="space-y-6">
            {result ? (
              <ResultsDisplay 
                ...
                assignedDealer={urlParams.assignedDealer}
                productInterest={urlParams.productInterest}
